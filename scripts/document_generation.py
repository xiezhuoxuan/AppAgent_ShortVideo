    ...
    for i in range(1, step + 1):
        ...
        if status:
            ...
            with open(doc_path, "w") as outfile:
                outfile.write(json.dumps(doc_content))
            doc_count += 1
            print_with_color(f"文档生成完毕，已保存至 {doc_path}", "yellow")
        else:
...
for file in txt_files:
    ...
    with open(sms_path, 'r') as f:
        ...
        if content['tap']:
            print_with_color(f"点赞功能的描述：{content['tap']}", "green")
        elif content['v_swipe']:
            print_with_color(f"上滑功能的描述：{content['v_swipe']}", "green")
        else:
            print_with_color("说明书格式有误！", 'red')
