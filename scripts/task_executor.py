...
with open(user_desc_fn, 'r') as f:
    content = json.loads(f.read())
    user_desc = content['user_desc']
    user_like = content['user_like']
    user_dislike = content ['user_dislike']
...
# 大模型给的决定是点击某一个区域，这个函数负责把区域翻译成坐标，这样才能适配adb
...
print_with_color(f"启动用时 {time.time()-eval(s_time)} 秒", 'cyan')    
while round_count < configs["MAX_ROUNDS"] or not new_video:
    ...
    if new_video:
        ...
        tmp_str = "Reason: "+reason.replace('\n', " ") + "\n"
        print_with_color(tmp_str, 'green')
        ## action
        # ui_doc = ""
    ...
    if status:
        ...
        if action == "tap":
            ...
            if verbose:
                ...
                print_with_color(f"点赞用时 {t2}秒", 'cyan')
            new_video = False
            if ret == "ERROR":
            ...
            t2 = time.time() - t1
            print_with_color(f"上滑用时 {t2}秒", 'cyan')
            new_video = True
            if ret == "ERROR":
